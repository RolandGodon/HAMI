...
"""
Snakemake is a workflow management system written in Python. It facilitates the creation and execution of complex data analysis pipelines, 
particularly in bioinformatics and computational biology. It is associated to a config.yaml file as well as a environment.yaml file. 
Please check snakemake Tutorial for futher explications: (https://snakemake.readthedocs.io/en/stable/tutorial/tutorial.html)

Here HAMI_PIPELINE.smk is a snakemake workflow associated to HAMI framework. 
It is associated to a config file (config.yaml) and an environement file (HAMI_environment.yaml).
...
rule Clean_and_Chimeres:
    ...
    Then it identifies chimeric sequences using the isBimeraDenovo(...) function in the dada2 library. By default the number of threads is set to 2, more can be used.
...
rule Separate_BARCODING_METABARCODING_data:
    ...
    Your samples thus need to be discrimated on the basis of their name. Please use alphabectic prefix for it e.g : METABAR0001 
    Also pipeline is developped to handle duplicate of samples. Discrimination between duplicate will be done using suffix : e.g : METABAR0001A / METABAR0001B
...
rule Pseudogene_Filter_and_reduce_redundancy:
    ...
    A pseudogene (NUMTs) filtering step is then performed as recommended in the literature (see: SONG et al., 2018 - Proceedings of the National Academy of Sciences) on the occurence file.
    It means that affiliation associated to sequences whose reading frame has shifted -as a result of indel events - or in which stop codons have appeared are discarded. 
    ...
    The occurrences file, free of pseudogenes, is subsequently processed to reduce data redundancy caused by multiple OTUs associated with the same species, reflecting intraspecific variability.
    ...
    shell:
        """
        python3 SCRIPTS/pseudogene_and_redundancy.py  {input.ABUNDANCE} {input.TAX} {params.prefix} {params.target_taxon} {params.outputdir} {params.name} {params.fragment} {params.seq_length} {params.reading_frame} {params.threshold} {params.codon_stop} 
        """
